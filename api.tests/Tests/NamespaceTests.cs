...
namespace RevloDB.API.Tests
{
    public class NamespaceControllerTests : IClassFixture<ApiTestAppFactory>
    {
        ...
        [Fact]
        public async Task GetNamespaceByName_WithNoAccess_ShouldReturnNotFound()
        {
            var owner = await _userUtility.GetNamespaceOwnerAsync();
            var readonlyUser = await _userUtility.GetReadonlyUserAsync();
            var testNamespace = await CreateTestNamespaceAsync(owner);
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", readonlyUser.AccessToken);
            var response = await _client.GetAsync($"/api/v1/namespace/by-name/{testNamespace.Name}");
